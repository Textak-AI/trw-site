...
function Sec({children,bg=C.white,pad="64px 0",id}){return <section id={id} style={{background:bg,padding:pad}}><div className="r-section-inner" style={{maxWidth:960,margin:"0 auto",padding:"0 28px"}}>{children}</div></section>;}
...
function H1({children}){return <h1 className="r-h1" style={{fontFamily:F.h,fontSize:38,fontWeight:700,color:C.black,margin:"0 0 20px",lineHeight:1.18}}>{children}</h1>;}
function H2({children,id}){return <h2 className="r-h2" id={id} style={{fontFamily:F.h,fontSize:28,fontWeight:700,color:C.black,margin:"0 0 16px",lineHeight:1.25}}>{children}</h2>;}
...
function Home({nav}){return <>
  ...
  style={{borderRadius:0,filter:"brightness(0.3)",objectPosition:"70% 35%"}}/></div>
    ...
    <div style={{position:"relative",zIndex:2,maxWidth:960,margin:"0 auto",padding:"80px 28px 72px"}}>
      ...
      <div className="r-btn-row" style={{display:"flex",gap:14}}><Btn primary onClick={()=>document.getElementById('pulse-check')?.scrollIntoView({behavior:'smooth'})}>Take the Free Pulse Check</Btn><Btn onClick={()=>nav("speakup")}>What Is Speak Up Culture?</Btn></div>
    </div>
  </section>
  ...
  <Sec>
    <div className="r-grid-2" style={{display:"grid",gridTemplateColumns:"5fr 4fr",gap:40}}>
      <div>
        ...
        <div className="r-btn-row" style={{display:"flex",gap:12,marginTop:8}}><Btn primary onClick={()=>nav("process")}>See Our Process</Btn><Btn onClick={()=>nav("classii")}>For Class II & III</Btn></div>
      </div>
  ...
  <Sec bg={C.cream}>
    <div className="r-grid-2" style={{display:"grid",gridTemplateColumns:"1fr 2fr",gap:40,alignItems:"center"}}>
      ...
      <div>
        ...
        <div className="r-btn-row" style={{display:"flex",gap:12}}><Btn primary onClick={()=>nav("about")}>Full Bio</Btn><Btn onClick={()=>nav("insights")}>Read Her Column</Btn></div>
      </div>
    </div>
  </Sec>
  ...
  <Sec>
    ...
    <div className="r-grid-2" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,marginTop:16}}>
      {[
        {img:IMG.roadAhead,title:"The Road Ahead: Putting It All Together",p:"Part 4 of 4",d:"Feb 2026",url:"https://therailway.us/the-road-ahead-2/"},
        {img:IMG.fieldLeadership,title:"Lead Yourself First: The Trinity of Excellence",p:"Part 3 of 4",d:"Jan 2026",url:"https://therailway.us/lead-yourself-first/"},
        {img:IMG.leaderTalking,title:"Leading Through Uncertainty: What's Your Style?",p:"Part 2 of 4",d:"Oct 2025",url:"https://therailway.us/leading-through-uncertainty-part-2/"},
        {img:IMG.freightTrain,title:"How Leaders Navigate Uncertainty and Come Out Stronger",p:"Part 1 of 4",d:"Aug 2025",url:"https://therailway.us/leaders-navigate-uncertainty/"},
      ].map((a,i)=><a key={i} href={a.url} target="_blank" rel="noopener noreferrer" style={{textDecoration:"none",background:C.cream,borderRadius:8,overflow:"hidden",cursor:"pointer",display:"block",transition:"transform 0.15s"}}>
...
/* ══════ INSIGHTS ══════ */
function Insights({nav}){const articles=[{img:IMG.roadAhead,t:"The Road Ahead: Putting It All Together",c:"Leadership Series",d:"Feb 2026",s:"Part 4/4",url:"https://therailway.us/the-road-ahead-2/"},{img:IMG.fieldLeadership,t:"Lead Yourself First: Trinity of Excellence",c:"Leadership Series",d:"Jan 2026",s:"Part 3/4",url:"https://therailway.us/lead-yourself-first/"},{img:IMG.leaderTalking,t:"Leading Through Uncertainty: What's Your Style?",c:"Leadership Series",d:"Oct 2025",s:"Part 2/4",url:"https://therailway.us/leading-through-uncertainty-part-2/"},{img:IMG.freightTrain,t:"How Leaders Navigate Uncertainty",c:"Leadership Series",d:"Aug 2025",s:"Part 1/4",url:"https://therailway.us/leaders-navigate-uncertainty/"},{img:IMG.safetyCulture,t:"Rail Safety by the Numbers: What FRA Data Reveals",c:"Data & Analysis",d:"2025",s:"",url:"https://therailway.us/rail-safety-human-factors/"},{img:IMG.yardTraining,t:"What Is Speak Up Culture — And Why Every Railroad Needs One",c:"Speak Up Culture",d:"Mar 2026",s:"",url:"https://therailway.us/speak-up-culture-rail-safety/"},{img:IMG.teamTraining,t:"Why Transformation Beats Training Every Time",c:"Culture Change",d:"Mar 2026",s:"",url:"https://therailway.us/transformation-vs-training/"},{img:IMG.railwayTraining,t:"The ROI of Safety Culture: Building the Business Case",c:"ROI & Data",d:"Mar 2026",s:"",url:"https://therailway.us/roi-safety-culture/"}];
return <><Sec pad="48px 0 56px"><Bc items={PAGES.insights.bc} nav={nav}/><Eye>The Rail Way™ Column & Pillar Articles</Eye><H1>{PAGES.insights.h1}</H1><P s={{maxWidth:560}}>Expert perspectives on rail leadership, safety culture, and operational excellence — from Contributing Editor Pauline Lipkewich and The Rail Way™ team.</P></Sec><Sec bg={C.cream}><div className="r-grid-2" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>{articles.map((a,i)=><a key={i} href={a.url} target="_blank" rel="noopener noreferrer" style={{textDecoration:"none",background:C.white,borderRadius:8,overflow:"hidden",cursor:"pointer",display:"block"}}><div style={{height:140,overflow:"hidden"}}><Img src={a.img} alt={a.t}/></div><div style={{padding:"14px 18px"}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><span style={{fontFamily:F.b,fontSize:10,color:C.rust,textTransform:"uppercase",letterSpacing:"1px"}}>{a.c}</span><span style={{fontFamily:F.b,fontSize:10,color:C.grayLt}}>{a.d}</span></div><h3 style={{fontFamily:F.h,fontSize:16,color:C.black,margin:0,lineHeight:1.3}}>{a.t}</h3>{a.s&&<span style={{fontFamily:F.m,fontSize:10,color:C.grayLt}}>{a.s}</span>}</div></a>)}</div></Sec></>;}

/* ══════ CLASS II & III ══════ */
function ClassII({nav}){return <>
  <section style={{position:"relative",minHeight:260,overflow:"hidden"}}><div style={{position:"absolute",inset:0}}><Img src={IMG.roadAhead} alt="Freight train on open track" className="r-img-hero" style={{borderRadius:0,filter:"brightness(0.3)"}}/></div><div style={{position:"relative",zIndex:1,maxWidth:960,margin:"0 auto",padding:"48px 28px"}}><Bc items={PAGES.classii.bc} nav={nav}/><Eye>For Regional & Short-Line Operators</Eye><h1 className="r-h1" style={{fontFamily:F.h,fontSize:38,fontWeight:700,color:C.white,margin:"0 0 16px",lineHeight:1.18}}>{PAGES.classii.h1}</h1><p style={{fontFamily:F.b,fontSize:17,color:"rgba(255,255,255,0.7)",maxWidth:560,margin:0}}>Same safety challenges as Class I — with a fraction of the resources. Right-sized programs that deliver results.</p></div></section>
